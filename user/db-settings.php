...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_POST['saveChanges'])) {
        ...
        $query->bindParam(":adminId", $userId);
        ...
        if (!$query->execute()) {
            ...
            header("Location: db-settings.php");
            exit();
        }
    }
            ...
            if ($stored_password === $current_password || password_verify($current_password, $stored_password)) {
                ...
                if ($new_password === $confirm_password) {
                    ...
                    if ($query->execute()) {
                        ...
                        $errorInfo = $query->errorInfo();
                        echo "<script>alert('Error changing password: " . $errorInfo[2] . "');</script>"; // Display error if update fails
                    }
                } else {
