...
function validateForm() {
  const priceInput = document.querySelector('input[name="ePrice"]');
  const pricePattern = /^\d+(\.\d{1,2})?$/;
  if (!pricePattern.test(priceInput.value)) {
    alert('Please enter a valid price (up to 2 decimal places)');
    return false;
  }
  return true;
}
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  // Set the initial min value for the departure date based on the arrival date
  setMinDepartureDate();

  // Add an event listener to handle changes in the arrival date
  ...
  function setMinDepartureDate() {
    ...
    if (arrivalDate) {
      // Set the departure date's min to the arrival date
      departureDateInput.min = arrivalDate;

      // If departure date is less than arrival date, clear or reset it
