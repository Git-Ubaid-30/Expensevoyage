...
if (isset($_POST['inp']) && isset($_POST['userId'])) {
    $searchInput = $_POST['inp'];
    $userId = $_POST['userId'];

    $query = $pdo->prepare("SELECT * FROM trips WHERE user_id = :userId AND (destination LIKE :search OR name LIKE :search)");
    ...
    echo '<tr><td colspan="7">Invalid request.</td></tr>'; // Handle invalid requests
}
?>
