            ...
            try {
                ...
                if ($trip) {
                    ...
                    if ($ePrice > $currentBudget) {
                        ...
                        $insertExpenseQuery = $pdo->prepare("INSERT INTO expenses (t_id, price, c_id, note, user_id) 
    VALUES (:tId, :ePrice, :cId, :eNote, :userId)");
                        ...
                        $insertExpenseQuery->bindParam("userId", $_SESSION['userId']); // Bind user_id from session
...
if (isset($_POST['addTrip'])) {
    ...
    if (empty($t_destErr) && empty($t_budErr) && empty($t_arrivalErr) && empty($t_departErr) && empty($t_nameErr)) {
        $query = $pdo->prepare("
            INSERT INTO trips (destination, budget, arrival_date, departure_date, name, user_id) 
            VALUES (:t_destination, :t_budget, :t_arrival, :t_departure, :t_name, :userId)
        ");
        ...
        $query->bindParam("userId", $_SESSION['userId']); // Bind user_id from session
...
if (isset($_POST["addCat"])) {
    ...
    if (empty($errCat) && empty($errdesc)) {
        try {
            ...
            $checkQuery = $pdo->prepare("SELECT COUNT(*) FROM categories WHERE name = :name AND user_id = :id");
            $checkQuery->bindParam("name", $cName);
            $checkQuery->bindParam("id", $_SESSION['userID']);
            $checkQuery->execute();
            ...
            if ($categoryCount > 0) {
                ...
                $query = $pdo->prepare("INSERT INTO categories (name, description, user_id) VALUES (:name, :desc, :userId)");
                $query->bindParam("name", $cName);
                $query->bindParam("desc", $cDesc);
                $query->bindParam("userId", $_SESSION['userId']); // Bind userId
...
$cName = $cPhone = $cEmail = $cSubject = $cMessage = "";
$cNameErr = $cPhoneErr = $cEmailErr = $cSubjectErr = "";
...
if (isset($_POST["submitform"])) {
    ...
    if (empty($cNameErr) && empty($cPhoneErr) && empty($cEmailErr) && empty($cSubjectErr)) {

        $query = $pdo->prepare("INSERT INTO contact (name, phone, email, subject, message) VALUES (:cName, :cPhone, :cEmail, :cSubject, :cMessage)");
        ...
        echo '<script>alert("Thank you for submit")</script>';
